...
async function cropScreenshot(inputPath, outputPath) {
    try {
        ...
        if (metadata.width !== 1280 || metadata.height !== 800) {
            ...
            console.log(`  Scaled crop for ${metadata.width}x${metadata.height}: left=${cropConfig.left}, top=${cropConfig.top}, width=${cropConfig.width}, height=${cropConfig.height}`);
        }
...
async function main() {
    ...
    console.log(`Crop config: left=${CROP_CONFIG.left}, top=${CROP_CONFIG.top}, width=${CROP_CONFIG.width}, height=${CROP_CONFIG.height}\n`);
